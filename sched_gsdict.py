...
class Propagator():
    ...
    def check_visibility(self):
        ...
        # find new reading
        for i in range(len(self.ground_stations.ecef)):

            # set each ground station to not visible
            self.ground_stations.visible[i] = 0
            if (elevation_dot_product(self.r_ecef,ground_stations.ecef[i],self.earth) >  0.0):

                # if the ground station is visible, set it to visible
                self.ground_stations.visible[i] = 1
...
from gs import ground_stations
