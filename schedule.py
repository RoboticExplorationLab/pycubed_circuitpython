...
def dynamics(x,Earth):
    ...
    * pos[2] * (3 - 5 * pos[2] ** 2 / pos_norm ** 2)
    ...
    xdot = np.array([vel[0],vel[1],vel[2],accel[0],accel[1],accel[2]])
...
def ECEF2ANG(r_ecef,station):
	...
	diff_normalized = diff/(np.vector.sqrt(diff[0] ** 2 + diff[1] ** 2 + diff[2] ** 2))
	r_station_normalized = r_station/(np.vector.sqrt(r_station[0] ** 2 + r_station[1] ** 2 + r_station[2] ** 2))
